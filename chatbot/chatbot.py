...
class Chatbot:
    ...
    class TestMode:
        ...
        # The chatbot runs on background and can regularly be called to predict
        # something
        DAEMON = 'daemon'
    ...
    def parseArgs(args):
        ...
        globalArgs.add_argument('--test',
                                nargs='?',
                                choices=[
                                    Chatbot.TestMode.ALL, Chatbot.TestMode.INTERACTIVE, Chatbot.TestMode.DAEMON],
        ...
        globalArgs.add_argument(
            '--seed', type=int, default=None, help='random seed for replication')

        # Dataset options
        datasetArgs = parser.add_argument_group('Dataset options')
        datasetArgs.add_argument('--corpus', choices=['cornell', 'opensubs', 'qq'], default='cornell',
                                 help='corpus on which extract the dataset. Only two corpus available right now (cornell and opensubs)')  # TODO: Replace by cst registered in textdata
        # The samples are computed from the corpus if it does not exist
        # already. There are saved in \'data/samples/\'
        ...
        nnArgs = parser.add_argument_group(
            'Network options', 'architecture related option')
        nnArgs.add_argument('--hiddenSize', type=int, default=256,
                            help='number of hidden units in each RNN cell')
        nnArgs.add_argument('--numLayers', type=int,
                            default=2, help='number of rnn layers')
        ...
        trainingArgs.add_argument(
            '--numEpochs', type=int, default=30, help='maximum number of epochs to run')
        trainingArgs.add_argument('--saveEvery', type=int, default=1000,
                                  help='nb of mini-batch step before creating a model checkpoint')
        trainingArgs.add_argument(
            '--batchSize', type=int, default=10, help='mini-batch size')
        trainingArgs.add_argument(
            '--learningRate', type=float, default=0.001, help='Learning rate')
    ...
    def main(self, args=None):
        ...
        # tf.logging.set_verbosity(tf.logging.INFO) # DEBUG, INFO, WARN
        ...
        if '12' in tf.__version__:  # HACK: Solve new tf Saver V2 format
            self.saver = tf.train.Saver(
                max_to_keep=200, write_version=1)  # Arbitrary limit ?
        else:
        ...
        # Also fix seed for random.shuffle (does it works globally for all
        # files ?)

        # Running session
        self.sess = tf.Session(config=tf.ConfigProto(
            # Allows backup device for non GPU-available operations (when
            # forcing GPU)
        ...
        # Reload the model eventually (if it exist.), on testing mode, the
        # models are not loaded here (but in predictTestset)
    ...
    def mainTrain(self, sess):
        ...
        try:  # If the user exit while training, we still try to save the model
            for e in range(self.args.numEpochs):
                ...
                # Warning: Will overflow if an epoch takes more than 24 hours,
                # and the output isn't really nicer
                if np.abs(loss) < 0.001:
                    break
                print("Epoch finished in {},loss:{}".format(toc - tic, loss))
        ...
        for modelName in sorted(modelList):  # TODO: Natural sorting
            ...
            with open(saveName, 'w') as f:
                ...
                for line in tqdm(lines, desc='Sentences'):
                    ...
                    f.write(predString)
                print(
                    'Prediction finished, {}/{} sentences ignored (too long)'.format(nbIgnored, len(lines)))
    ...
    def mainTestInteractive(self, sess):
        ...
        # TODO: Log the questions asked for latter re-use (merge with
        # test/samples.txt)
        ...
        while True:
            ...
            if self.args.verbose:
                print(self.textData.batchSeq2str(
                    questionSeq, clean=True, reverse=True))
    ...
    def loadQQEmbedding(self, sess):
        ...
        # New model, we load the pre-trained word2vec data and initialize
        # embeddings
        vector_size = 1024
        initW = np.random.uniform(-0.25, 0.25,
                                  (len(self.textData.word2id), vector_size))
        for word in self.textData.model.vocab:
            initW[self.textData.word2id[word]] = self.textData.model[word]
        ...
        if self.args.embeddingSize < vector_size:
            ...
            initW = np.dot(U[:, :self.args.embeddingSize], S[
                           :self.args.embeddingSize, :self.args.embeddingSize])
    ...
    def loadEmbedding(self, sess):
        ...
        if self.args.corpus == "qq":
            return self.loadQQEmbedding(sess)
        # Fetch embedding variables from model
        with tf.variable_scope("embedding_rnn_seq2seq/RNN/EmbeddingWrapper", reuse=True):
            em_in = tf.get_variable("embedding")
        with tf.variable_scope("embedding_rnn_seq2seq/embedding_rnn_decoder", reuse=True):
    ...
    def managePreviousModel(self, sess):
        ...
        if os.listdir(self.modelDir):
            ...
            # Analysing directory content
                ...
                print('Restoring previous model from {}'.format(modelName))
                # Will crash when --reset is not activated and the model has
                # not been saved yet
                self.saver.restore(sess, modelName)
            ...
            if self.args.reset:
                fileList = [os.path.join(self.modelDir, f)
    ...
    def loadModelParams(self):
        ...
        if not self.args.reset and not self.args.createDataset and os.path.exists(configName):
            ...
            # We need to restore the model length because of the textData
            # associated and the vocabulary size (TODO: Compatibility mode
            # between different maxLength)
            ...
            self.args.initEmbeddings = config[
                'Network'].getboolean('initEmbeddings')
            softmaxSamples = config['Network'].getint('softmaxSamples')
            if softmaxSamples is not None:
                self.args.softmaxSamples = config[
                    'Network'].getint('softmaxSamples')

            # No restoring for training params, batch size or other non model
    ...
    def saveModelParams(self):
        ...
        config['Training (won\'t be restored)'][
            'learningRate'] = str(self.args.learningRate)
        config['Training (won\'t be restored)'][
    ...
    def getDevice(self):
        ...
        if self.args.device == 'cpu':
            ...
            print('Warning: Error in the device name: {}, use the default device'.format(
                self.args.device))
            return None
